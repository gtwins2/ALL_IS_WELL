			   ...
			   <div class="list-area">
				<table>
					...
					<c:forEach items="${sendList}" var="list">
						<tr class="sendList-row" data-sendMail-no="${list.no}">
   ...
   <footer>
	...
	<script type="text/javascript">
	    ...
	    $(document).ready(function () {
	        $('.sendList-row').click(function (e) {
	            // 클릭한 요소가 not-clickable 클래스를 가진 요소가 아니라면
	            if (!$(e.target).hasClass('not-clickable')) {
	            	const sendMailNo = $(this).attr('data-sendMail-no');
	            	console.log("넘어갈 번호: " + sendMailNo);

	                window.location.href = "/app/mail/mailDetail?no=" + sendMailNo;
	            }
	        });
	    });
			    ...
			    if (checkboxes.length > 0) {
			        ...
			        checkboxes.forEach(checkbox => {
			            if (checkbox.value !== "selectAll") {
			                ...
			                const mailNo = checkbox.closest('.sendList-row').getAttribute('data-sendMail-no');

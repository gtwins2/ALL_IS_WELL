    ...
    <style>
        * {
		...
		#send {
		    ...
		    align-items: center;
		    cursor: pointer;
		    box-shadow: 0px 2px px rgba(0, 0, 0, 0.2);
		    color: white;
		}
		...
		#menu {
		  ...
		  padding: 10px;
		  border-radius: 5px;
		  transform: translateY(calc(-900% - 10px));
...
<body>
	<div class="main-wrap">
        <div class="title-area">
            	  <c:choose>
				    <c:when test="${sessionScope.loginMember.no eq chattingDetailList[0].receiverNo}">
					    <img src="/app/resources/static/profile/${chattingDetailList[0].senderProfile}" alt="프로필 사진" id="title-profile">
					    <div class="title-name-area">
						<div id="title-name">${chattingDetailList[0].senderName}(${chattingDetailList[0].senderDepartmentName} ${chattingDetailList[0].senderPositionName})</div>
						<c:if test="${chattingDetailList[0].senderAttendanceStatus eq 'O'}">
							<div id="title-status">출근</div>
						</c:if>
						<c:if test="${chattingDetailList[0].senderAttendanceStatus eq 'X' or empty chattingDetailList[0].senderAttendanceStatus}">
				    ...
				    <c:otherwise>
					    <img src="/app/resources/static/profile/${chattingDetailList[0].receiverProfile}" alt="프로필 사진" id="title-profile">
					    <div class="title-name-area">
						<div id="title-name">${chattingDetailList[0].receiverName}(${chattingDetailList[0].receiverDepartmentName} ${chattingDetailList[0].receiverPositionName})</div>
						<c:if test="${chattingDetailList[0].receiverAttendanceStatus eq 'O'}">
							<div id="title-status">출근</div>
						</c:if>
						<c:if test="${chattingDetailList[0].receiverAttendanceStatus eq 'X' or empty chattingDetailList[0].receiverAttendanceStatus}">
							<div id="title-status">출근 전</div>
						</c:if>
					    </div>
				    </c:otherwise>
        ...
        <div class="content-area">
        	<%-- <c:forEach items="${chattingDetailList}" var="list">
        		<c:if test="${list.receiverNo eq list.writerNo}">
	        		<div class="chat-me">
			               <img src="/app/resources/static/profile/${list.receiverProfile}" alt="" class="profile-image">
			                <div class="content">
			                    <div class="name">${list.receiverName}</div>
        		...
        		<c:if test="${list.senderNo eq list.writerNo}">
    		...
    		<c:forEach items="${chattingDetailList}" var="list">
    <c:choose>
        <c:when test="${sessionScope.loginMember.no eq list.writerNo}">
            <div class="chat-opponent">
                <div class="content">
                    <div class="textbox">${list.messengerContent}</div>
                    <div class="date">${list.mailEnrollDate}</div>
        ...
        <c:otherwise>
            <div class="chat-me">
                <img src="/app/resources/static/profile/${list.senderNo eq sessionScope.loginMember.no ? list.receiverProfile : list.senderProfile}" alt="" class="profile-Image">
                <div class="content">
                    <div class="name">${list.senderNo eq sessionScope.loginMember.no ? list.receiverName : list.senderName}</div>
                    <div class="textbox">${list.messengerContent}</div>
                    <div class="date">${list.mailEnrollDate}</div>
                </div>
    ...
    let chattingRoomNo = '<%= request.getParameter("chattingRoomNo") %>';
    console.log('chattingRoomNo:', chattingRoomNo);
	
    let quitChattingBtn = document.querySelector("#quitChattingBtn");
    
    quitChattingBtn.addEventListener("click", function(){
    	alert("채팅방을 나가셨습니다.");
    	location.href="/app/chatting/quitChatting?chattingRoomNo="+chattingRoomNo;
    });
    ...
    ws.onmessage = function(event) {
        ...
        const confirmYn = message.confirmYn;

	...
	<main id="wrap">
		...
		<div class="main-area">
			...
			<div class="title-area">
				<br> <br> <span id="title" style="width: 200px; font-size: 30px;">당직
					리스트</span>
			...
			<div class="number-area">
				<c:if test="${pv.listCount > 10}">
					<c:if test="${pv.currentPage > 1}">
						<a
							href="select?page=1&searchType=${svo.getSearchType()}&searchValue=${svo.getSearchValue()}">&laquo;</a>
						<a
							href="select?page=${pv.currentPage - 1}&searchType=${svo.getSearchType()}&searchValue=${svo.getSearchValue()}">&lt;</a>
					</c:if>
					<c:set var="finalEndPage"
						value="${pv.endPage > pv.maxPage ? pv.maxPage : pv.endPage}" />
					<c:forEach var="i" begin="${pv.startPage}" end="${finalEndPage}"
						step="1">
						<c:choose>
							...
							<c:otherwise>
								<a
									href="select?page=${i}&searchType=${svo.getSearchType()}&searchValue=${svo.getSearchValue()}">${i}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${pv.maxPage > pv.currentPage}">
						<a
							href="select?page=${pv.currentPage + 1}&searchType=${svo.getSearchType()}&searchValue=${svo.getSearchValue()}">&gt;</a>
						<a
							href="select?page=${pv.maxPage}&searchType=${svo.getSearchType()}&searchValue=${svo.getSearchValue()}">&raquo;</a>
					</c:if>
				</c:if>
			</div>
